  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: () =>
      queryClient.invalidateQueries({
        queryKey: ["election", String(election.id)],
      }),
  });
    ...
    <div className="space-y-5 py-2">
      ...
      {["completed", "active"].includes(election.status) && (
        ...
        <div className="flex items-center justify-between rounded-xl border px-4 py-3 bg-card">
          ...
          >
            {election.is_result_published
              ? t("results.published")
              : t("results.unpublished")}
          </span>
        </div>
      )}

      {/* Public visibility toggle — only for org_admin / election_admin / super_admin */}
      {canToggle && ["completed", "active"].includes(election.status) && (
        <div className="flex items-center justify-between rounded-xl border px-4 py-3 bg-card">
          <div className="flex items-start gap-3">
            <div
              className={`mt-0.5 p-1.5 rounded-lg ${election.is_public_result ? "bg-indigo-100 text-indigo-600" : "bg-muted text-muted-foreground"}`}
            >
          ...
          >
            {toggleMutation.isPending ? (
              ...
              t("results.make_private")
            ) : (
              t("results.publish_publicly")
            )}
          </Button>
        </div>
      )}
      ...
      {canToggle && (
        <div className="flex items-center justify-between rounded-xl border px-4 py-3 bg-card">
          <div className="flex items-start gap-3">
            <div
              className={`mt-0.5 p-1.5 rounded-lg ${election.is_public_voter_list ? "bg-teal-100 text-teal-600" : "bg-muted text-muted-foreground"}`}
            >
              <Users size={15} />
            </div>
            <div>
      ...
      {["completed", "active"].includes(election.status) && (
        <Button
          onClick={() =>
            navigate(`${basePath}/elections/${election.id}/results`)
          }
        >
          <BarChart2 size={14} className="mr-1.5" />{" "}
          {t("results.view_full_results")}
        </Button>
      )}
      {!["completed", "active"].includes(election.status) && (
        <div className="text-sm text-muted-foreground py-4">
